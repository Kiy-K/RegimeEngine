...
# ═══════════════════════════════════════════════════════════════════════════ #
# Sector Positions — loaded from real geographic coordinates                  #
# Generated by gui/generate_map.py from Natural Earth data                   #
# ═══════════════════════════════════════════════════════════════════════════ #

import json as _json

_ASSETS = os.path.join(os.path.dirname(os.path.abspath(__file__)), "assets")
_MAP_IMAGE_PATH = os.path.join(_ASSETS, "map_western_europe.png")
_POSITIONS_PATH = os.path.join(_ASSETS, "sector_positions.json")

# Load sector positions from JSON (real geo → pixel mapping)
# Add Y_OFFSET because map image is drawn at y=60 (below top bar)
_MAP_Y_OFFSET = 60
if os.path.exists(_POSITIONS_PATH):
    with open(_POSITIONS_PATH) as _f:
        _raw = _json.load(_f)
        SECTOR_POS = {int(k): (v[0], v[1] + _MAP_Y_OFFSET) for k, v in _raw.items()}
else:
    print("WARNING: sector_positions.json not found. Run: .venv/bin/python gui/generate_map.py")
    SECTOR_POS = {i: (200 + (i % 7) * 70, 100 + (i // 7) * 80) for i in range(35)}

# Sea zone approximate centers (positioned on the map image + offset)
SEA_ZONE_POS = {
    0: (460, 398 + _MAP_Y_OFFSET),   # Dover Strait
    1: (380, 440 + _MAP_Y_OFFSET),   # Western Channel
    2: (480, 280 + _MAP_Y_OFFSET),   # North Sea
    3: (200, 250 + _MAP_Y_OFFSET),   # Irish Sea
    4: (280, 520 + _MAP_Y_OFFSET),   # Bay of Biscay
    5: (120, 140 + _MAP_Y_OFFSET),   # North Atlantic
}
...
class AirStripOneGUI:
    def __init__(self, seed=42, max_turns=100, speed=1.0):
        ...
        # Load map image
        self.map_image = None
        if os.path.exists(_MAP_IMAGE_PATH):
            try:
                raw = pygame.image.load(_MAP_IMAGE_PATH)
                self.map_image = pygame.transform.scale(raw, (700, 740))
    ...
    def _draw_map(self):
        ...
        if self.map_image is not None:
            self.screen.blit(self.map_image, (0, 60))
        else:
            sea_rect = pygame.Rect(0, 60, 700, 740)
